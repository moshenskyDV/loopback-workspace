...
readOperations.prototype.readModelProperty = function(id, cb) {
  var parts = id.split('.');
  var facet = parts[0];
  var modelName = parts[1];
...
readOperations.prototype.readModelMethod = function(id, cb) {
  var parts = id.split('.');
  var facet = parts[0];
  var modelName = parts[1];
...
readOperations.prototype.readModelRelation = function(id, cb) {
  var parts = id.split('.');
  var facet = parts[0];
  var modelName = parts[1];
  var relationName = parts[2];
  var folder = config.ModelDefaultDir;
  var file = path.join(loader().getDirectory(), facet, folder, modelName + '.json');
  fs.readJson(file, function(err, data) {
    if(err) return cb(err);
    var relations = data.relations;
    var config = relations[relationName];
    cb(null, config);
  });
}
